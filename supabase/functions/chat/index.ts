...
function getSystemInstruction(
  ...
  if (subjectLower.includes('впр') || subjectLower.includes('vpr')) {
    ...
    const classMatch = subject.match(/(\d+)\s*класс/);
    const vprClass = classMatch ? classMatch[1] : classLevel || '?';
    const vprSubject = subject.replace(/впр\s*\d*\s*класс:?\s*/i, '').trim();
    
    examInstruction = `
    РЕЖИМ: ПОДГОТОВКА К ВПР (Всероссийская проверочная работа)
    Класс: ${vprClass}
    Предмет: ${vprSubject || subject}
    
    СТРУКТУРА ЗАДАНИЙ ВПР:
    ВПР содержит задания разного типа:
    - Задания с кратким ответом (число, слово, последовательность)
    - Задания с развёрнутым ответом (объяснение, решение, рассуждение)
    - Задания на соответствие и выбор правильного ответа
    - Задания с использованием таблиц, графиков, карт, изображений
    
    ПРАВИЛА ГЕНЕРАЦИИ ЗАДАНИЙ:
    1. Генерируй задания СТРОГО по формату реальных ВПР для ${vprClass} класса
    2. Каждое задание должно иметь НОМЕР (как в настоящей работе)
    3. Указывай МАКСИМАЛЬНЫЙ БАЛЛ за каждое задание
    4. После ответа ученика — оценивай по КРИТЕРИЯМ ВПР:
       - Полный правильный ответ = максимальный балл
       - Частично правильный = часть баллов (если применимо)
       - Неправильный = 0 баллов
    5. Разбирай ошибки подробно, показывай правильное решение
    6. Засекай ОРИЕНТИРОВОЧНОЕ ВРЕМЯ на задание
    
    ТИПИЧНЫЕ ОШИБКИ для ${vprClass} класса — обращай на них внимание!
    
    ФОРМАТ ОТВЕТА ПРИ ГЕНЕРАЦИИ ЗАДАНИЯ:
    📋 Задание №X (максимум Y баллов)
    [текст задания]
    
    После ответа ученика:
    ✅ или ❌ Оценка: Z/Y баллов
    [разбор]`;
  } else if (subjectLower.includes('огэ') || subjectLower.includes('oge')) {
    const ogeSubject = subject.replace(/огэ:?\s*/i, '').replace(/oge:?\s*/i, '').trim();
    
    examInstruction = `
    РЕЖИМ: ПОДГОТОВКА К ОГЭ (Основной государственный экзамен, 9 класс)
    Предмет: ${ogeSubject || subject}
    
    СТРУКТУРА ОГЭ:
    ОГЭ состоит из двух частей:
    - Часть 1: задания с кратким ответом (тестовая часть)
    - Часть 2: задания с развёрнутым ответом
    
    ПРАВИЛА ГЕНЕРАЦИИ ЗАДАНИЙ:
    1. Генерируй задания СТРОГО по формату КИМ ОГЭ (контрольно-измерительные материалы)
    2. Нумеруй задания как в реальном ОГЭ
    3. Указывай ПЕРВИЧНЫЙ БАЛЛ за каждое задание
    4. Объясняй КРИТЕРИИ оценивания для заданий части 2
    5. Помогай ученику понять ТИПИЧНЫЕ ЛОВУШКИ в заданиях
    6. После ответа — переводи баллы в примерную оценку (шкала ФИПИ)
    
    КЛЮЧЕВЫЕ ОСОБЕННОСТИ ОГЭ по ${ogeSubject}:
    - Все материалы соответствуют кодификатору и спецификации ФИПИ
    - Обращай внимание на формулировки — они должны быть идентичны реальным КИМ
    - Учи правильно ОФОРМЛЯТЬ ответы (это важно для получения баллов)
    
    ФОРМАТ ОТВЕТА:
    📝 Задание №X (Y баллов) | Часть Z
    [текст задания в формате КИМ]`;
  } else if (subjectLower.includes('егэ') || subjectLower.includes('ege')) {
    const egeSubject = subject.replace(/егэ:?\s*/i, '').replace(/ege:?\s*/i, '').trim();
    
    examInstruction = `
    РЕЖИМ: ПОДГОТОВКА К ЕГЭ (Единый государственный экзамен, 11 класс)
    Предмет: ${egeSubject || subject}
    
    СТРУКТУРА ЕГЭ:
    ЕГЭ состоит из двух частей:
    - Часть 1: задания с кратким ответом (1-й первичный балл за задание)
    - Часть 2: задания с развёрнутым ответом (2-4 первичных балла за задание)
    
    ПРАВИЛА ГЕНЕРАЦИИ ЗАДАНИЙ:
    1. Генерируй задания СТРОГО по формату КИМ ЕГЭ
    2. Нумерация заданий как в реальном ЕГЭ (по спецификации ФИПИ текущего года)
    3. Указывай ПЕРВИЧНЫЙ БАЛЛ за каждое задание
    4. Для части 2 — подробные КРИТЕРИИ оценивания (К1, К2, К3...)
    5. После ответа — показывай перевод первичных баллов в ТЕСТОВЫЕ (по шкале)
    6. Обращай внимание на ПОРОГ (минимальный балл) для предмета
    
    КЛЮЧЕВЫЕ ОСОБЕННОСТИ ЕГЭ по ${egeSubject}:
    - Строгое соответствие кодификатору ФИПИ
    - Формулировки должны быть ИДЕНТИЧНЫ реальным КИМ
    - Учи стратегии распределения ВРЕМЕНИ (3-3.5 часа на работу)
    - Объясняй, как ОФОРМЛЯТЬ ответы на бланке
    - Обрати внимание на задания-ЛОВУШКИ
    
    ФОРМАТ ОТВЕТА:
    🎓 Задание №X (Y первичных баллов) | Часть Z
    [текст задания в формате КИМ]`;
  }
