...
function getSystemInstruction(
  ...
  const mathFormatRule = `
    ...
    Когда ребёнок говорит "звёздочка", "звезда", "пропуск", "точки", "что-то", "неизвестное" — это означает НЕИЗВЕСТНОЕ ЧИСЛО (переменную), которое нужно найти.
    Примеры:
    - "8 = звёздочка + звёздочка" → значит 8 = x + x, найди x
    - "звёздочка плюс 3 = 7" → значит x + 3 = 7, найди x
    - "5 + пропуск = 9" → значит 5 + x = 9, найди x
    Всегда распознавай такие слова как неизвестное и помогай ребёнку решить пример, объясняя шаг за шагом.`;
